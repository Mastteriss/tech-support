...
### Способ 1: Автоматический (вместе с запуском)
При запуске через `start.bat` или `start.sh` тесты запускаются автоматически перед стартом приложения.

### Способ 2: Отдельный запуск
```bash
# Запустить все тесты
python test_run.py
...
# Или по отдельности:
cd server
python test_models.py    # Тесты моделей (16 тестов)
python test_api.py       # Тесты API (8 тестов)
### Проверка сервера:
...
**Гришин Иван Анатольевич**
- Группа: Б262532.
- GitHub: [@Mastteriss](https://github.com/Mastteriss)

---

**© 2026 - Система технической поддержки**
