...
# с автоматической проверкой, установкой и тестами
...
echo -e "${BLUE}--- Тесты моделей (test_models.py) ---${NC}"
python test_models.py
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅ Тесты моделей ПРОЙДЕНЫ!${NC}"
else
    echo -e "${RED}❌ Тесты моделей НЕ ПРОЙДЕНЫ!${NC}"
fi
echo ""

echo -e "${BLUE}--- Тесты API (test_api.py) ---${NC}"
python test_api.py
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅ Тесты API ПРОЙДЕНЫ!${NC}"
else
    echo -e "${RED}❌ Тесты API НЕ ПРОЙДЕНЫ!${NC}"
fi
echo ""
